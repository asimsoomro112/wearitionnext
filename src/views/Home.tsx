...
const CategoryGrid = ({ items }: { items?: any[] }) => (
  <section className="py-16 md:py-32 px-6 lg:px-12 w-full max-w-[1600px] mx-auto z-10 relative bg-background">
     ...
     <div className="bento-grid">
        {(items || []).map((cat, i) => (
           <Link 
             href={cat.link || `/shop?category=${cat.name?.toLowerCase()}`} 
             key={i} 
        ...
        return (
          <section key={section.id || index} className="hero-section relative w-full h-[100dvh] min-h-[650px] md:min-h-[800px] overflow-hidden flex items-center px-5 md:px-[6%] text-white bg-[#0b0d12]">
            ...
            <div className="parallax-bg absolute inset-0 z-[1]" style={{
              background: `linear-gradient(90deg, rgba(0,0,0,0.88) 0%, rgba(0,0,0,0.72) 35%, rgba(0,0,0,0.28) 60%, rgba(0,0,0,0.1) 100%), url("${heroImg.src || heroImg}")`,
            ...
            <div className="hero-content relative z-[2] max-w-[820px] parallax-foreground pt-4 md:pt-0 pb-20 md:pb-0 scale-[0.85] sm:scale-95 md:scale-100 origin-left">
              ...
              >
                <MagneticButton strength={0.2} as="div" className="inline-block">
                  <Link href="/shop" className="group relative inline-flex items-center gap-[14px] px-[42px] py-[22px] bg-foreground text-background overflow-hidden rounded-full font-bold tracking-[0.2em] text-[0.9rem] md:text-[1rem] transition-all duration-500 hover:text-accent">
